...
function payLine(code){ const w=WAGE[soc6(code)]; if(!w||!w.m) return ''; const range=(w.lo&&w.hi)?`<span class="range">typical ${fmtK(w.lo)}–${fmtK(w.hi)}</span>`:''; return `<p class="pay"><span class="med">$${w.m.toLocaleString()}</span><span class="peryr">/yr median</span>${range}</p>`; }
...
function dirPay(p){ const rep=p[5]; if(!rep) return ''; const w=WAGE[soc6(rep)]; if(!w||!w.m) return ''; const t=(OCC[rep]||{}).t; return `<p class="pay"><span class="med">$${w.m.toLocaleString()}</span><span class="peryr">/yr median${t?' — '+esc(t):''} <em>(representative)</em></span></p>`; }
