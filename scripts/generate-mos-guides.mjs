...
// O*NET occupation descriptions (what each civilian job actually entails), keyed by O*NET-SOC code.
const OCC_DESC=(()=>{ try{ return JSON.parse(fs.readFileSync(path.join(ROOT,'data','onet-occupations.json'),'utf8')); }catch(e){ return {}; } })();
...
function jobCard(r,i,mosTitle){
  const cl=clusterFor(r.code), d=OCC_DESC[r.code], mt=esc(mosTitle);
  const why = i===0 ? 'Your '+mt+' duties map most directly to this role.'
    : (cl ? 'Draws on the '+esc(cl[0].toLowerCase())+' side of your '+mt+' experience.'
          : 'A U.S. Department of Labor–recognized match for your '+mt+' experience.');
  return '<article>'+(i===0?'<span class="tag">Closest match</span>':'')+'<h3>'+esc(r.title)+'</h3>'+
...
function page(branch, entry){
  ...
  const jobsSection=civRoles.length?`<section><h2>Civilian jobs that match ${esc(entry.code)}</h2><p class="lede">These are the civilian jobs the U.S. Department of Labor maps to ${esc(entry.code)}, ordered from the closest match. Think of them as a floor, not a ceiling—see what each role involves and pays, start where your ${esc(entry.title)} experience already fits, and build toward the rest.</p><div class="jobs">${civRoles.map((r,i)=>jobCard(r,i,entry.title)).join('')}</div><p class="source">Source: <a href="https://www.onetcenter.org/crosswalks.html" rel="nofollow noopener" target="_blank">O*NET / DMDC Military Crosswalk</a>, August 2024.</p></section>`:'';
