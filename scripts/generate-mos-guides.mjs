...
// Navy and Coast Guard share some rating abbreviations (BM, OS, IT, ...). Branch-qualify
// the newer side so each gets its own page while pre-existing URLs stay put.
...
function slugFor(branch,code){return (SLUG_PREFIX[branch+'|'+code]||'')+slug(code);}
...
// Real BLS OEWS 2023 median/10th/90th-pct annual wages, keyed by 6-digit SOC.
const WAGE=(()=>{ try{ return JSON.parse(fs.readFileSync(path.join(ROOT,'data','bls-wages.json'),'utf8')); }catch(e){ return {}; } })();
// O*NET Job Zone (1–5) = how much education/training a role typically needs.
const JZ=(()=>{ try{ return JSON.parse(fs.readFileSync(path.join(ROOT,'data','onet-jobzones.json'),'utf8')); }catch(e){ return {}; } })();
const JZTEXT={1:'Little prep — a few days to a few months of training',2:'High-school diploma plus short on-the-job training',3:'Associate degree, apprenticeship, or vocational training',4:'Bachelor’s degree typically expected',5:'Graduate or professional degree'};
const soc6=code=>String(code||'').split('.')[0];
const fmtK=n=>'$'+Math.round(n/1000)+'k';
const fmtEmp=n=>n>=1e6?(n/1e6).toFixed(1).replace(/\.0$/,'')+'M':(n>=1e3?Math.round(n/1e3)+'K':''+n);
function payLine(code){ const w=WAGE[soc6(code)]; if(!w||!w.m) return ''; const range=(w.lo&&w.hi)?`<span class="range">typical ${fmtK(w.lo)}–${fmtK(w.hi)}</span>`:''; const emp=w.e?`<span class="emp">~${fmtEmp(w.e)} U.S. jobs</span>`:''; return `<p class="pay"><span class="figure"><span class="med">$${w.m.toLocaleString()}</span><span class="peryr">/yr median</span></span>${range}${emp}</p>`; }
function entryLine(code){ const z=JZ[code]||JZ[soc6(code)+'.00']; if(!z||!JZTEXT[z]) return ''; return `<p class="prep"><strong>Typical entry:</strong> ${JZTEXT[z]}</p>`; }
// O*NET/CareerOneStop "Bright Outlook" — occupations projected to grow rapidly and/or have
// numerous openings. Baked from the CareerOneStop API (only bright codes stored).
const OUTLOOK=(()=>{ try{ return JSON.parse(fs.readFileSync(path.join(ROOT,'data','onet-outlook.json'),'utf8')); }catch(e){ return {}; } })();
function outlookBadge(code){ const cat=OUTLOOK[code]||OUTLOOK[soc6(code)+'.00']||OUTLOOK[soc6(code)]; if(!cat) return ''; const label=String(cat).replace(/Rapid Growth/i,'Rapid growth').replace(/Numerous Job Openings/i,'many openings').replace(/New & Emerging/i,'new & emerging'); return `<p class="outlook"><span class="star">★</span> Bright Outlook${label&&label!=='Bright'?` — <em>${esc(label)}</em>`:''}</p>`; }
// Direction cards carry a representative BLS median only when the curation supplies an
// accurate per-path SOC (path[5]); no guessing from the linked page (that overpromised).
function dirPay(p){ const rep=p[5]; if(!rep) return ''; const w=WAGE[soc6(rep)]; if(!w||!w.m) return ''; const t=(OCC[rep]||{}).t; return `<p class="pay"><span class="figure"><span class="med">$${w.m.toLocaleString()}</span><span class="peryr">/yr median</span></span><span class="repof">${t?esc(t)+' ':''}<em>(representative)</em></span></p>`; }

// Action-oriented transition programs — the sources O*NET can't give you. Shown on every
// page (universal) plus a field-specific set, so even a combat-arms vet sees a real on-ramp.
const VET_PROGRAMS_UNIVERSAL=[
 ['DoD SkillBridge','Train or intern with a civilian employer during your last ~180 days on active duty—at full military pay.','https://skillbridge.osd.mil/'],
 ['Registered Apprenticeships','Earn a paycheck while you train for a trade or technical role; USMAP can credit the military training you already have.','https://www.apprenticeship.gov/'],
 ['Hiring Our Heroes','Corporate fellowships and hiring events that place transitioning service members and military spouses.','https://www.hiringourheroes.org/'],
 ['Onward to Opportunity','Free career training and nationally recognized certifications for service members, veterans, and spouses.','https://ivmf.syracuse.edu/o2o/'],
 ['VA VET TEC','VA-funded high-tech training (IT, cyber, data, software) that does not touch your GI Bill entitlement.','https://www.va.gov/education/about-gi-bill-benefits/how-to-use-benefits/vettec-high-tech-program/'],
 ['CareerOneStop (U.S. DOL)','Free local career centers plus certification and state-by-state license finders for any field.','https://www.careeronestop.org/']
];
const VET_PROGRAMS_BY_FAMILY={
 technology:[['Microsoft Software & Systems Academy','17-week tech training (cloud, cyber, development) with a built-in hiring network.','https://military.microsoft.com/mssa/'],['SANS VetSuccess','Scholarship cybersecurity training and industry certifications for veterans.','https://www.sans.org/cybertalent/'],['VetsInTech','Free tech education, entrepreneurship, and employment programs.','https://vetsintech.co/']],
 engineering:[['Helmets to Hardhats','Free placement into registered apprenticeships across the building and construction trades.','https://helmetstohardhats.org/']],
 mechanical:[['Helmets to Hardhats','Registered apprenticeships in the skilled trades—your discipline is the prerequisite.','https://helmetstohardhats.org/'],['Amazon military roles','Maintenance, logistics, and operations pathways that actively recruit veterans.','https://www.amazon.com/militaryroles']],
 logistics:[['Amazon military roles','Logistics, operations, and driver pathways that actively recruit veterans.','https://www.amazon.com/militaryroles']]
};
...
function experienceBands(branch, title, rt){
  ...
  if(rt==='O') return [
    ['Company-Grade Officer · O-1–O-3','Translate planning, technical judgment, briefings, team leadership, and the people and resources you were accountable for.'],
  ...
  const junior='Junior Enlisted · E-1–E-4';
  const nco=naval?'Petty Officer · E-5–E-6':'Non-Commissioned Officer · E-5–E-6';
  const senior=naval?'Chief Petty Officer · E-7–E-9':'Senior NCO · E-7–E-9';
  return [
...
// Normalize any band label (new "Tier · Grade", old "E-5–E-6 / NCO", or bare "E-1–E-4")
// into [tierName, gradeBadge] so the rank cards read intentionally instead of "E-1–E-4".
const TIER_BY_GRADE={'E-1':'Junior Enlisted','E-5':'Non-Commissioned Officer','E-7':'Senior NCO','O-1':'Company-Grade Officer','O-4':'Field-Grade Officer','O-6':'Senior Officer','WO1':'Warrant Officer','CW3':'Chief Warrant Officer','CW5':'Senior Chief Warrant Officer'};
const TIER_BY_SUFFIX={'NCO':'Non-Commissioned Officer','Senior NCO':'Senior NCO','Petty Officer':'Petty Officer','Chief':'Chief Petty Officer','Chief Petty Officer':'Chief Petty Officer'};
function normBand(name){
  name=String(name).trim();
  if(name.includes(' · ')){ const [t,g]=name.split(' · '); return [t.trim(),g.trim()]; }
  if(name.includes(' / ')){ const [g,suf]=name.split(' / '); return [TIER_BY_SUFFIX[suf.trim()]||suf.trim(), g.trim()]; }
  const first=(name.match(/^(E-\d|O-\d|WO\d|CW\d)/)||[])[0];
  const tier=first?TIER_BY_GRADE[first]:'';
...
function jobCard(r,kind,mosTitle,handpicked){
  ...
  const why = r.why ? esc(r.why) : (kind==='closest'
    ? (handpicked
        ? (cl ? 'Your '+mt+' experience maps directly onto the '+clx+' side of this role.'
              : 'A civilian role your '+mt+' experience maps to directly.')
  ...
  return '<article class="'+kind+'"><h3>'+esc(r.title)+'</h3>'+
    payLine(r.code)+outlookBadge(r.code)+entryLine(r.code)+
...
function page(branch, entry){
  ...
  const closestGrid=civRoles.length?`<h3 class="jobgroup">${civRoles.length>1?'Closest matches':'Closest match'}</h3><div class="jobs">${civRoles.map(r=>jobCard(r,'closest',entry.title,jobsCurated)).join('')}</div>`:'';
  const relatedGrid=relatedMatches.length?`<h3 class="jobgroup">Related roles worth exploring</h3><div class="jobs">${relatedMatches.map(r=>jobCard(r,'related',entry.title,jobsCurated)).join('')}</div>`:'';
  const catchall=esc((rawCiv[0]||{}).title||'All Other'), noXwalk=!rawCiv.length;
  const jobsLede=jobsCurated
    ? (noXwalk
        ? `There's no one-to-one civilian crosswalk for ${esc(entry.code)}—the Department of Labor doesn't map a single job title to it. These are the specific civilian roles ${esc(entry.title)} experience genuinely maps to: what each pays, what it involves, and why it fits.`
        : `The federal crosswalk lists only a broad “${catchall}” bucket for ${esc(entry.code)}—so here are the specific civilian roles ${esc(entry.title)} experience genuinely maps to. See what each involves, what it pays, and why it fits.`)
  ...
  const jobsSource=jobsCurated
    ? (noXwalk
        ? `Roles selected from the O*NET occupations that fit ${esc(entry.title)} experience; the Department of Labor doesn't publish a one-to-one crosswalk for this code. Pay is BLS OEWS (May 2023). See the <a href="https://www.onetcenter.org/crosswalks.html" rel="nofollow noopener" target="_blank">O*NET crosswalk</a>.`
        : `Roles selected from the O*NET occupations that fit ${esc(entry.title)} experience; the Department of Labor's own crosswalk lists only the broad “${catchall}” category for this code. Pay is BLS OEWS (May 2023). See the <a href="https://www.onetcenter.org/crosswalks.html" rel="nofollow noopener" target="_blank">O*NET crosswalk</a>.`)
    : `Source: <a href="https://www.onetcenter.org/crosswalks.html" rel="nofollow noopener" target="_blank">O*NET / DMDC Military Crosswalk</a> and O*NET related-occupations data, August 2024. Pay is BLS OEWS (May 2023).`;
  ...
  const dirLede=noCross?`There's no single civilian job titled “${esc(entry.title)}”—the U.S. Department of Labor doesn't publish a one-to-one crosswalk for this code. That is not a dead end. These are the fields where ${esc(entry.title)} experience is genuinely valued, each with a straight answer on why it fits and exactly what it takes to get there.`:(curated?.paths?'Adjacent and stretch fields where your experience gives you a credible starting point.':'Broader fields your experience can open, with honest credential notes.');
  const dirSection=dirPaths?`<section><h2>${dirTitle}</h2><p class="lede">${dirLede}</p><div class="paths">${dirPaths.map((p,i)=>`<article><div class="path-head"><span>${String(i+1).padStart(2,'0')}</span><div><small>${esc(p[1])}</small><h3>${esc(p[0])}</h3></div></div>${dirPay(p)}<p class="why"><strong>Why it fits:</strong> ${esc(p[2])}</p><p class="bridge"><strong>What it takes:</strong> ${esc(p[3])}</p><a href="/${p[4]}">Explore this route →</a></article>`).join('')}</div></section>`:'';
  const people=({Army:'Soldiers','Air Force':'Airmen',Navy:'Sailors',Marines:'Marines','Coast Guard':'Coast Guardsmen','Space Force':'Guardians'})[branch]||'people';
  const progs=[...VET_PROGRAMS_UNIVERSAL,...(VET_PROGRAMS_BY_FAMILY[family]||[])];
  const progSection=`<section class="programs"><h2>Programs built to hire &amp; train veterans</h2><p class="lede">You don't have to make this jump alone. These programs exist specifically to move service members into civilian careers—most are free, and several pay you while you train.</p><div class="proglist">${progs.map(p=>`<a class="prog" href="${esc(p[2])}" rel="nofollow noopener" target="_blank"><h3>${esc(p[0])} <span class="ext">↗</span></h3><p>${esc(p[1])}</p></a>`).join('')}</div></section>`;
  return `<!doctype html><html lang="en"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>${esc(entry.code)} ${esc(entry.title)} Civilian Careers by Rank | Veteran Career Path</title><meta name="description" content="Explore civilian, federal, contractor, and training pathways for ${esc(branch)} ${esc(entry.code)} ${esc(entry.title)}, with different guidance for junior and senior experience."><link rel="canonical" href="https://veterancareerpath.com/mos/${slugFor(branch,entry.code)}.html"><link rel="stylesheet" href="/vcp-styles.css"><link rel="preconnect" href="https://fonts.googleapis.com"><link rel="preconnect" href="https://fonts.gstatic.com" crossorigin><link href="https://fonts.googleapis.com/css2?family=Bebas+Neue&family=Inter:wght@400;500;600;700&display=swap" rel="stylesheet"><style>${MOS_CSS}</style></head><body><nav><a class="brand" href="/"><img src="https://veterancareerpath.com/img/optimized/logo-96.webp" width="28" height="28" alt="Veteran Career Path"><span>Veteran Career Path</span></a><a class="backlink" href="/${HUB[branch]}">← ${esc(branch)} job codes</a></nav><header><img class="emblem" src="https://veterancareerpath.com/img/optimized/${EMBLEM[branch]}.webp" alt="${esc(branch)} emblem" loading="eager"><p class="eyebrow">${esc(branch)} · ${esc(entry.code)}</p><h1>${esc(entry.title)}</h1><p>${esc(introText)}</p></header><main>${jobsSection}${dirSection}<section><h2>Your rank changes the story</h2><p class="lede">Use the level that reflects what you actually did—not rank alone. Two people with the same code and very different experience should not submit the same résumé.</p><div class="bands">${bands.map(([name,text])=>{const [tier,grade]=normBand(name);return `<article class="band">${grade?`<span class="grade">${esc(grade)}</span>`:''}<h3>${esc(tier)}</h3><p>${esc(text)}</p></article>`;}).join('')}</div></section><section class="playbook"><h2>Turn ${esc(entry.code)} into proof employers trust</h2><p class="lede">A translated title isn't enough. These four moves turn ${esc(entry.title)} experience into evidence a civilian hiring manager can act on.</p><ol class="steps"><li><h3>Show what you ran</h3><p>List the systems, vehicles, equipment, and tools you personally operated as ${esc(entry.title)}—plus qualifications, certifications, and the recurring high-stakes tasks you owned.</p></li><li><h3>Put numbers on leadership</h3><p>${esc(people)} led, dollar value of equipment you were accountable for, workload, readiness rates, and safety or quality outcomes. Scope beats adjectives.</p></li><li><h3>Close the credential gap early</h3><p>For any target role, name the license, degree, or software it requires and start now—your GI Bill and credentialing assistance can cover most of it.</p></li><li><h3>Search by capability, not code</h3><p>Recruiters rarely search “${esc(entry.code)}.” Search job boards by the equipment, skills, and outcomes above as much as by “${esc(entry.title)}.”</p></li></ol></section>${progSection}<p class="note"><strong>How to use this:</strong> Treat these as targets, not limits. Your exact duties depend on your assignments, so lead with what you actually did—and remember that regulated careers require their own license, certification, or degree.</p><div class="actions"><a href="/app.html">Build a capability-based résumé</a><a class="secondary" href="/federal-jobs-search.html">Search federal jobs</a></div>${citesHtml}</main><footer>Built for veterans who want more than a one-job crosswalk. · <a href="/contact.html">Suggest a correction or pathway</a><br><span class="attrib">Career outlook data powered by <a href="https://www.careeronestop.org/" rel="nofollow noopener" target="_blank">CareerOneStop</a> (U.S. Department of Labor). Pay data: BLS OEWS.</span></footer></body></html>`;
}

const MOS_CSS=`*{box-sizing:border-box}body{margin:0;font-family:Inter,-apple-system,Segoe UI,Roboto,Arial,sans-serif;color:#1e2a3a;background:#eef2f7;line-height:1.6;-webkit-font-smoothing:antialiased}a{color:#12305e}nav{height:56px;padding:0 6%;display:flex;align-items:center;justify-content:space-between;background:#0a1628;color:#fff;position:sticky;top:0;z-index:10}nav .brand{display:flex;align-items:center;gap:.55rem;text-decoration:none;flex-shrink:0}nav .brand img{width:28px;height:28px;object-fit:contain}nav .brand span{font-family:'Bebas Neue',sans-serif;font-size:.9rem;letter-spacing:.12em;color:#f0c040}nav .backlink{color:#f0c040;text-decoration:none;font-weight:700;font-size:.88rem}header{position:relative;overflow:hidden;padding:4rem 6% 3.4rem;color:#fff;text-align:center;background:radial-gradient(900px 380px at 50% -25%,#24457a,transparent),linear-gradient(140deg,#0a1628,#12294f 62%,#183c6e)}header:after{content:"";position:absolute;left:0;right:0;bottom:0;height:4px;background:linear-gradient(90deg,#f0c040,#e0a92e)}header>*{max-width:1000px;margin-left:auto;margin-right:auto;position:relative}.emblem{width:66px;height:66px;object-fit:contain;display:block;margin:0 auto .7rem;filter:drop-shadow(0 4px 12px rgba(0,0,0,.45))}.eyebrow{color:#f4cf6a;font-size:.75rem;font-weight:700;letter-spacing:.18em;text-transform:uppercase}h1{font-family:"Bebas Neue",sans-serif;font-size:clamp(2.8rem,7vw,5rem);line-height:.98;letter-spacing:.02em;margin:.4rem auto .8rem}header>p:not(.eyebrow){font-size:1.1rem;color:#cfe0f5;max-width:780px}.chips{display:flex;flex-wrap:wrap;gap:.5rem;margin-top:1.4rem;justify-content:center}.chips span{background:rgba(240,192,64,.1);border:1px solid rgba(240,192,64,.35);color:#f6e8c6;border-radius:999px;padding:.4rem .85rem;font-size:.8rem;font-weight:500}main{max-width:1040px;margin:0 auto;padding:2.4rem 5% 3rem}section{margin-bottom:2.6rem}h2{font-family:"Bebas Neue",sans-serif;letter-spacing:.03em;font-size:1.75rem;color:#12294f;margin:0 0 .3rem;padding-left:.7rem;border-left:4px solid #e0a92e}.lede{color:#54657c;max-width:820px;margin:.2rem 0 1.2rem}.official{list-style:none;padding:0;margin:0;display:grid;gap:.6rem}.official li{background:#fff;border:1px solid #e2eaf4;border-left:4px solid #c8960a;border-radius:10px;padding:.8rem 1rem;font-weight:600;color:#17263a;box-shadow:0 2px 10px rgba(12,26,48,.05)}.soc{font-size:.68rem;font-weight:600;background:#eef3f9;color:#3a5070;padding:.15rem .45rem;border-radius:5px;margin-left:.4rem}.source{font-size:.8rem;color:#6a7c92;margin-top:.9rem}.bands,.paths,.evidence{display:grid;grid-template-columns:repeat(auto-fit,minmax(250px,1fr));gap:1.1rem}.bands article,.evidence div,.paths article{background:#fff;border:1px solid #e4ebf3;border-radius:14px;padding:1.3rem;box-shadow:0 3px 14px rgba(12,26,48,.05)}.bands article{border-top:3px solid #12294f}.bands article>span,.path-head>span{font-family:"Bebas Neue",sans-serif;font-size:1.3rem;color:#c8960a;letter-spacing:.04em}.bands h3,.paths h3,.evidence h3{margin:.25rem 0;color:#12294f;font-size:1.05rem}.bands p,.evidence p,.paths article>p{color:#4a5a70;font-size:.92rem}.paths article{display:flex;flex-direction:column;transition:transform .15s,box-shadow .15s,border-color .15s}.paths article:hover{transform:translateY(-3px);box-shadow:0 12px 28px rgba(12,26,48,.11);border-color:#cdd9ea}.path-head{display:flex;gap:.7rem;align-items:baseline}.path-head small{text-transform:uppercase;letter-spacing:.08em;color:#7086a0;font-size:.68rem;font-weight:700}.paths .why{font-size:.9rem;color:#42556e;margin:.1rem 0 .55rem;line-height:1.55}.paths .why strong{color:#12294f}.bridge{background:#f2f6fb;border:1px solid #e3edf7;padding:.6rem .7rem;border-radius:8px;font-size:.85rem;color:#33455e;margin-top:auto}.bridge strong{color:#12294f}.paths a{font-weight:700;color:#155799;text-decoration:none;margin-top:.7rem;display:inline-block}.paths a:hover{text-decoration:underline}.paths article:first-child{border:1.5px solid #e0a92e;box-shadow:0 8px 24px rgba(224,169,46,.14)}.evidence h3{font-size:1rem}.jobs{display:grid;grid-template-columns:repeat(auto-fit,minmax(250px,1fr));gap:1.1rem}.jobs article{background:#fff;border:1px solid #e4ebf3;border-top:3px solid #c8960a;border-radius:14px;padding:1.2rem;box-shadow:0 3px 14px rgba(12,26,48,.05);transition:transform .15s,box-shadow .15s}.jobs article:hover{transform:translateY(-3px);box-shadow:0 12px 28px rgba(12,26,48,.12)}.jobs h3{margin:.1rem 0 .3rem;color:#12294f;font-size:1.05rem;line-height:1.3}.jobgroup{font-family:Inter,sans-serif;font-size:.72rem;font-weight:800;letter-spacing:.12em;text-transform:uppercase;color:#8a6a12;margin:1.4rem 0 .7rem;padding:0}.jobgroup:first-of-type{margin-top:.4rem}.jobs article.related{border-top-color:#9fb4d0}.pay{margin:.15rem 0 .4rem;display:flex;align-items:baseline;flex-wrap:wrap;gap:.3rem .5rem}.pay .figure{display:inline-flex;align-items:baseline;gap:.28rem;white-space:nowrap}.pay .med{font-family:"Bebas Neue",sans-serif;font-size:1.4rem;letter-spacing:.02em;color:#1a7a40;line-height:1;white-space:nowrap}.pay .peryr{font-size:.76rem;color:#5a6b82;font-weight:600;white-space:nowrap}.pay .range{font-size:.72rem;color:#3f6b4c;background:#eef6f0;border:1px solid #d5e7db;border-radius:5px;padding:.12rem .45rem;white-space:nowrap}.pay .emp{font-size:.72rem;color:#425873;background:#eef2f8;border:1px solid #dbe4f0;border-radius:5px;padding:.12rem .45rem;white-space:nowrap}.pay .repof{font-size:.76rem;color:#5a6b82}.pay .repof em{font-style:normal;color:#8a97a8}.prep{font-size:.8rem;color:#5a6b82;margin:.1rem 0 .5rem}.prep strong{color:#12294f}.outlook{margin:.15rem 0 .4rem;font-size:.72rem;font-weight:700;color:#8a6a12;display:inline-flex;align-items:center;gap:.35rem;background:#fdf6e3;border:1px solid #f0d98c;border-radius:6px;padding:.16rem .5rem}.outlook .star{color:#e0a92e}.outlook em{font-style:normal;font-weight:600;color:#7a6410}.paths .pay{margin:.5rem 0 .1rem}.band .grade{display:inline-block;font-family:"Bebas Neue",sans-serif;letter-spacing:.06em;font-size:.9rem;color:#0a1628;background:linear-gradient(135deg,#f4cf6a,#e0a92e);padding:.16rem .6rem;border-radius:5px;margin-bottom:.55rem}.playbook .steps{list-style:none;counter-reset:step;padding:0;margin:0;display:grid;grid-template-columns:repeat(auto-fit,minmax(250px,1fr));gap:1.1rem}.playbook .steps li{counter-increment:step;background:#fff;border:1px solid #e4ebf3;border-top:3px solid #12294f;border-radius:14px;padding:1.3rem;box-shadow:0 3px 14px rgba(12,26,48,.05)}.playbook .steps li:before{content:counter(step,decimal-leading-zero);font-family:"Bebas Neue",sans-serif;font-size:1.7rem;color:#e0a92e;display:block;margin-bottom:.25rem;line-height:1}.playbook .steps h3{margin:.1rem 0 .35rem;color:#12294f;font-size:1.05rem}.playbook .steps p{color:#4a5a70;font-size:.92rem;margin:0}.programs{background:linear-gradient(140deg,#0a1628,#12294f);margin-left:-5%;margin-right:-5%;padding:2.2rem 5%;border-radius:0}.programs h2{color:#fff;border-left-color:#f0c040}.programs .lede{color:#cfe0f5;max-width:820px}.proglist{display:grid;grid-template-columns:repeat(auto-fit,minmax(250px,1fr));gap:.9rem}.prog{display:block;background:rgba(255,255,255,.06);border:1px solid rgba(240,192,64,.25);border-radius:12px;padding:1.1rem;text-decoration:none;transition:background .15s,border-color .15s,transform .15s}.prog:hover{background:rgba(255,255,255,.1);border-color:#f0c040;transform:translateY(-3px)}.prog h3{margin:0 0 .35rem;color:#f4cf6a;font-size:1rem;display:flex;justify-content:space-between;align-items:center;gap:.5rem}.prog .ext{color:#8fa8c8;font-size:.85rem}.prog p{margin:0;color:#cfe0f5;font-size:.86rem;line-height:1.5}.entails{color:#48586e;font-size:.9rem;line-height:1.5;margin:.3rem 0 .4rem}.why{font-size:.85rem;color:#5a6b82;margin:.2rem 0 .6rem;line-height:1.5}.why strong{color:#12294f}.joblinks{display:flex;flex-direction:column;gap:.4rem;margin-top:.6rem}.joblinks a{font-weight:700;color:#155799;text-decoration:none;font-size:.9rem}.joblinks a:hover{text-decoration:underline}.note{color:#6a7c92;font-size:.84rem;line-height:1.6;margin:0 0 2.4rem;padding:.9rem 1.1rem;background:#f5f7fb;border:1px solid #e6ecf4;border-left:4px solid #a9c0dd;border-radius:10px}.note strong{color:#3f5573}.actions{display:flex;flex-wrap:wrap;gap:.7rem}.actions a{background:linear-gradient(135deg,#f4cf6a,#e0a92e);color:#0a1628;text-decoration:none;font-weight:700;padding:.85rem 1.35rem;border-radius:10px;box-shadow:0 5px 16px rgba(224,169,46,.3)}.actions .secondary{background:#12294f;color:#fff;box-shadow:none}.sources ul{padding-left:1.1rem;color:#54657c}.sources li{margin:.35rem 0}.sources a{color:#155799;font-weight:600}footer{background:#0a1628;color:#a9bcd6;padding:2rem 6%;text-align:center;font-size:.85rem}footer a{color:#f0c040}.attrib{font-size:.74rem;color:#7e93ad;display:inline-block;margin-top:.55rem}.attrib a{color:#f0c040}@media(max-width:600px){header{padding:3rem 1rem 2.6rem}main{padding:1.6rem 1rem 2.5rem}.bands,.paths,.evidence{grid-template-columns:1fr}}@media(max-width:480px){nav{padding:0 3%}nav .brand span{font-size:.8rem}}`;

let made=0;
for(const [branch,entries] of Object.entries(codesByBranch)){
  for(const entry of entries){fs.writeFileSync(path.join(ROOT,'mos',`${slugFor(branch,entry.code)}.html`),page(branch,entry));made++;}
  const hubPath=path.join(ROOT,HUB[branch]); let hub=fs.readFileSync(hubPath,'utf8');
  for(const entry of entries){
    const cur=CURATED[`${branch}|${entry.code}`], code=entry.code.replace(/[.*+?^${}()|[\]\\]/g,'\\$&'), href=`https://veterancareerpath.com/mos/${slugFor(branch,entry.code)}.html`, count=cur?.paths?.length||((official[`${branch}|${entry.code}`]||[]).filter(r=>!String(r.code||'').startsWith('55')).length||combinePaths(dataDrivenPaths(official[`${branch}|${entry.code}`]||[],entry.title),PATHS[cur?.family||familyFromRoles(official[`${branch}|${entry.code}`]||[])||familyFor(entry.title)]).length);
