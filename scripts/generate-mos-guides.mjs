...
function jobCard(r){
  const cl=clusterFor(r.code);
  return '<article><h3>'+esc(r.title)+'</h3>'+(r.code?'<span class="soc">O*NET '+esc(r.code)+'</span>':'')+
...
function page(branch, entry){
  ...
  const jobsSection=civRoles.length?`<section><h2>Civilian jobs that match ${esc(entry.code)}</h2><p class="lede">These civilian occupations are mapped to your code by the U.S. Department of Labor. Each links to real pay, job outlook, and the training or certification it requires—use them as specific targets, not limits.</p><div class="jobs">${civRoles.map(jobCard).join('')}</div><p class="source">Source: <a href="https://www.onetcenter.org/crosswalks.html" rel="nofollow noopener" target="_blank">O*NET / DMDC Military Crosswalk</a>, August 2024.</p></section>`:'';
