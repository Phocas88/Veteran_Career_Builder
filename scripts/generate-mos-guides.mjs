...
function payLine(code){ const w=WAGE[soc6(code)]; if(!w||!w.m) return ''; const range=(w.lo&&w.hi)?`<span class="range">typical ${fmtK(w.lo)}–${fmtK(w.hi)}</span>`:''; return `<p class="pay"><span class="figure"><span class="med">$${w.m.toLocaleString()}</span><span class="peryr">/yr median</span></span>${range}</p>`; }
