...
const CURATED={
  'Army|12B':{family:'engineering', intro:'Combat engineering is much broader than law enforcement. It combines construction, route and terrain reconnaissance, heavy equipment, demolition safety, obstacle reduction, logistics, and small-team problem solving. Those capabilities can open several civilian and government directions; they do not limit a veteran to one field.', capabilities:['Construction and survivability work','Route, bridge, terrain, and obstacle reconnaissance','Light and heavy engineer vehicle operation','Demolition planning and explosives safety','Job-site risk controls and standards','Equipment, materials, and team coordination']}
};
...
function page(branch, entry){
  const key=`${branch}|${entry.code}`, curated=CURATED[key], family=curated?.family||familyFor(entry.title), paths=PATHS[family], roles=official[key]||[], caps=curated?.capabilities||genericCapabilities(entry.title,family), bands=experienceBands(branch,entry.title);
...
for(const [branch,entries] of Object.entries(codesByBranch)){
  ...
  for(const entry of entries){
    const code=entry.code.replace(/[.*+?^${}()|[\]\\]/g,'\\$&'), href=`https://veterancareerpath.com/mos/${slug(entry.code)}.html`, count=(CURATED[`${branch}|${entry.code}`]?.family?PATHS[CURATED[`${branch}|${entry.code}`].family]:PATHS[familyFor(entry.title)]).length;
