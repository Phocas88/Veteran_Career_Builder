...
function page(branch, entry){
  const key=`${branch}|${entry.code}`, curated=CURATED[key], roles=official[key]||[], family=curated?.family||familyFromRoles(roles)||familyFor(entry.title), paths=curated?.paths||combinePaths(dataDrivenPaths(roles,entry.title),PATHS[family]), caps=curated?.capabilities||genericCapabilities(entry.title,family), bands=curated?.bands||experienceBands(branch,entry.title), introText=curated?.intro||(roles.length?`Your ${entry.title} experience maps to civilian fields like ${roles.slice(0,2).map(r=>r.title).join(' and ')}—and well beyond. Explore direct matches, adjacent fields, government work, and new directions where your background gives you a credible starting point.`:`Your ${entry.title} background is a portfolio of capabilities—not a sentence to one civilian job. Explore direct matches, adjacent fields, government work, and new directions where your experience gives you a credible starting point.`);
...
for(const [branch,entries] of Object.entries(codesByBranch)){
  ...
  for(const entry of entries){
    const cur=CURATED[`${branch}|${entry.code}`], code=entry.code.replace(/[.*+?^${}()|[\]\\]/g,'\\$&'), href=`https://veterancareerpath.com/mos/${slug(entry.code)}.html`, count=(cur?.paths||combinePaths(dataDrivenPaths(official[`${branch}|${entry.code}`]||[],entry.title),PATHS[cur?.family||familyFromRoles(official[`${branch}|${entry.code}`]||[])||familyFor(entry.title)])).length;
